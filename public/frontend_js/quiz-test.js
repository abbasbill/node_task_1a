...
async function askQuestion(totalQuizQuestions, counter, fromBack) {
  ...
  if (type == 1) {
      ...
      if (val.answer) {
        $("#typeMcq .answerInner").append(`
          <div class="mcqOptions">
            <button data-val="${val.answer}" data-id="${val.id}" 
            data-answer-value="${val.answer_value}" 
            data-bl-active='${val.black_list_actives}' 
            class="multiBtns mcqBtn">
            ${val.answer}
            </button>
          </div>
        `);
    ...
    ques.answers.forEach((val, index) => {
      if (val.answer) {
        $("#typeSelection .answerInner").append(`
          <div class="selectionOptions">
            <button data-val="${val.answer}" data-id="${val.id}"
             class="selectionBtns selectionBtn" 
     ...
     $("#typeSelection .answerInner").append(`
          <div class="selectionOptions">
            <button class="selectionBtns selectionBtn" 
...
function checkAllergie(answerValue) {
  // Read the data values from the clicked button
  const buttonLabel = $(`button[data-answer-value="${answerValue}"]`).attr(
    "data-val"
  );

  // Check if the button label matches any of the values
  const allergens = ["Banana", "Olive", "Sunflowers"];
  if (allergens.includes(buttonLabel)) {
    // Terminate the quiz if the allergen is detected
    terminateQuiz();
  }
}
async function terminateQuiz() {
 let result = await fetch("/api/v1/terminate", {
    method: "GET",
  }).then((res) => res.json())
    .then((data) => {
      if (data.success) {
    ...
    let {countdown, message } = result
  // Create a message element with a countdown
  const terminationMessage = $("<div class='termination-message'></div>");
  ...
  // Append the termination message to the body
  $("body").append(terminationMessage);
